...
const allowedOrigins = [
  'https://pwthor.site',
  'https://pw-m3u-d58f21da6afd.herokuapp.com', // Removed trailing slash
  'https://downloadm3u-dbe5522dc3e5.herokuapp.com',
  'http://localhost:28718',
  'http://localhost:8501' // Add this if you are testing Streamlit locally
];

const corsOptions = {
  origin: (origin, callback) => {
    // If no origin (like direct browser visit) or origin is in the list, allow it
    if (!origin || allowedOrigins.includes(origin)) {
      callback(null, true);
    } else {
      // Instead of throwing an Error (which causes a 500), 
      // just return 'false' to block the request via standard CORS behavior
      callback(null, false); 
    }
  },
  credentials: true,
